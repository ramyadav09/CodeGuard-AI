...
class GitHubService:
    ...
    def _get_headers(self, accept: str = "application/vnd.github.v3+json") -> dict[str, str]:
        headers = {"Accept": accept, "User-Agent": "CodeGuard-AI-App"}
        if self.token:
    ...
    async def get_pr_metadata(self, owner: str, repo: str, pr_number: int) -> PRMetadata:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            try:
                ...
                raise GitHubServiceError(
                    f"Network error connecting to GitHub API: {exc!s}", status_code=503
                )

            if response.status_code == 404:
                raise GitHubServiceError(
                    f"GitHub PR #{pr_number} not found in repository {owner}/{repo}",
                    status_code=404,
                )
            elif response.status_code == 401:
                raise GitHubServiceError(
                    "Unauthorized GitHub API request. Check your GITHUB_TOKEN.", status_code=401
                )
    ...
    async def get_pr_diff(self, owner: str, repo: str, pr_number: int) -> str:
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            try:
                ...
                raise GitHubServiceError(
                    f"Network error fetching PR diff: {exc!s}", status_code=503
                )
