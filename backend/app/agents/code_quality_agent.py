...
class CodeQualityAgent(BaseAgent):
    def __init__(self, ai_provider: AIProvider):
        super().__init__(
            ...
            description=(
                "Identifies code smells, duplication, high cognitive complexity, "
                "bad abstractions, and maintainability issues."
            ),
    ...
    async def analyze(
        ...
        system_instruction = (
            "You are an expert Code Quality AI Agent. Inspect code diffs for "
            "maintainability issues, code smells, excessive cognitive complexity, "
            "duplicated logic, dead code, poor naming, and anti-patterns.\n"
            "Rules:\n"
        ...
        try:
            result = await self.ai_provider.analyze_diff(
                prompt, system_instruction, QualityAgentOutput
            )
            ...
            for item in raw_findings:
                try:
                    ...
                    logger.warning(f"CodeQualityAgent finding validation skipped: {exc!s}")
            return valid_findings
        except Exception as exc:
            logger.error(f"CodeQualityAgent execution error: {exc!s}")
            return []
