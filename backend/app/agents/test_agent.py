...
class TestAnalysisAgent(BaseAgent):
    ...
    def __init__(self, ai_provider: AIProvider):
        super().__init__(
            ...
            description="Evaluates test coverage of modified/added code branches and suggests concrete unit test cases.",
            ai_provider=ai_provider
        )

    async def analyze(self, pr_metadata: PRMetadata, parsed_diff: ParsedDiff) -> List[FindingSchema]:
        system_instruction = (
            "You are an expert Test Analysis AI Agent. Analyze whether newly added or modified code paths in the PR diff "
            "have adequate unit/integration test coverage. Suggest concrete, runnable unit test code cases.\n"
            "Rules:\n"
        ...
        try:
            result = await self.ai_provider.analyze_diff(prompt, system_instruction, TestAgentOutput)
            ...
            for item in raw_findings:
                try:
                    ...
                    logger.warning(f"TestAnalysisAgent finding validation skipped: {str(exc)}")
            return valid_findings
        except Exception as exc:
            logger.error(f"TestAnalysisAgent execution error: {str(exc)}")
            return []
